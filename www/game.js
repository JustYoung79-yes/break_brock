...
function clampBallAngle(dx, dy) {
    ...
    // 수직에 가까우면(끼임 방지) 80도(수직 기준)로 강제
    const clampedTilt = tilt < 15 ? 80 : Math.max(20, Math.min(70, tilt));
...
function getBallDamage() {
    const base = currentStage >= 6 ? 90 : 10;
...
function hitBrick(brick, isBullet = false) {
    ...
    if (brick.isBoss && currentStage <= 4 && brick.hp <= 10 && !brick.mercyChoiceTriggered) {
